...
class Menu:
    ...
    def create_buttons(self):
        ...
        size = 180
        margin = 6
        topleft = (s.WINDOW_WIDTH - size - margin, s.WINDOW_HEIGHT - size - margin)
        ...
        self.chair_button_rect = generic_button_rect.move(
            0, self.rect.width / 2
        ).inflate(-button_margin, -button_margin)
...
class Button(pygame.sprite.Sprite):
    ...
    def switch(self):
        ...
        self.id = (
            0
            if self.id >= len(self.items["main" if self.main_active else "alt"])
            else self.id
        )
